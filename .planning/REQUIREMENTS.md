...
Acceptance Criteria:
- Recommendations are generated by Claude Vision API via Supabase Edge Function (never direct browser call)
...
Description:
All calls to the Claude Vision API must be proxied through a Supabase Edge Function to protect the API key.

Acceptance Criteria:
- Edge Function `style-match` deployed to Supabase
- `ANTHROPIC_API_KEY` stored as Edge Function secret — never in any frontend file
...
- Function calls Claude Vision API, receives structured JSON response, saves session, and returns product IDs + colour guidance
